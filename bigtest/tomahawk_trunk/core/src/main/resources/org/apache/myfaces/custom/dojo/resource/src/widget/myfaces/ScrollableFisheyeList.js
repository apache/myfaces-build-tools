...
dojo.widget.tags.addParseTreeHandler("dojo:ScrollableFisheyeList");
dojo.widget.tags.addParseTreeHandler("dojo:ScrollableFisheyeListItem");

dojo.widget.html.ScrollableFisheyeList = function(){
	dojo.widget.HtmlWidget.call(this);
}
dojo.inherits(dojo.widget.html.ScrollableFisheyeList, dojo.widget.HtmlWidget);
...
dojo.lang.extend(dojo.widget.html.ScrollableFisheyeList, {
	...
	onRightScroll: function(ev) {
		this.scrollerBegin = (this.scrollerBegin + this.visibleWindow < this.itemCount? (this.scrollerBegin + 1): this.scrollerBegin);
		this.initializePositioning();

	},

	onLeftScroll: function(ev) {
		this.scrollerBegin = (this.scrollerBegin > 0 ? (this.scrollerBegin - 1): 0);
		this.initializePositioning();
	
	},
	
	calcScrollerEnd : function() {
		return ((this.scrollerBegin + this.visibleWindow) > this.children.length ? 
			this.children.length  : this.scrollerBegin+this.visibleWindow);
	},
	...
	initializePositioning: function(){
		...
		var barItemCount = (this.itemCount > this.maxNoDisplayItems ? this.maxNoDisplayItems: this.itemCount);
		

		this.barWidth  = (this.isHorizontal ? this.visibleWindow : 1) * this.itemWidth;
	...
	paint: function(){
		...
		if(main_p < this.scrollerBegin)
			main_p = this.scrollerBegin;
		if(cen > 0 && cen  < this.scrollerBegin)
			cen = this.scrollerBegin;
		
		var offset = 0;

		var scrollerEnd = this.calcScrollerEnd();

		if (cen < this.scrollerBegin){
			main_p = this.scrollerBegin;
		} else if (cen > scrollerEnd - 1){
			main_p = scrollerEnd - 1;
		}else{
	...
	restoreState: function () {
		...
		//dojo.debug("val"+cookieValue);
		this.scrollerBegin = parseInt(cookieValue);
		
	},

	saveState: function (){
		var cookieName = this._getCookieName(0);
		dojo.io.cookie.setCookie(cookieName,this.scrollerBegin, null, null, null, null);
	},
...
//------------------------//
dojo.widget.html.ScrollableFisheyeListItem = function(){
	dojo.widget.HtmlWidget.call(this);
}
